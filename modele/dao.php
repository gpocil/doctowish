...
class praticien
{
    ...
    public static function read_all_prat()
        {
...
class rdv
{
    ...
    public static function read_rdv_user_id($id)//Récup liste RDV par utilisateur 
    {
        ...
        JOIN `praticien` ON `type_pec`.`prat_id` = `praticien`.`id`
        WHERE `user_id` = $id";
        $arr[] = connexion::query_fetch_all($sql);
        return $arr;
    }
    ...
    {        
        ...
        JOIN `user` ON `rdv`.`user_id` =`user`.`id`
        WHERE `type_pec`.`prat_id` = $id
        AND `type_pec`.`nom` = '$nom'";
        $arr[] = connexion::query_fetch_all($sql);
        return $arr;
    }
    public static function read_rdv_prat_id_date($id, $date)//Récup liste RDV pour le prat en fonction d'une date
    {        
        $sql = "SELECT `rdv`.`id`, `user`.`nom`, `user`.`prenom`, `type_pec`.`nom`, `heure_rdv`, `annulation`, `type_pec`.`prat_id` FROM `rdv` 
        JOIN `type_pec` ON `rdv`.`type_id` = `type_pec`.`id` 
        JOIN `user` ON `rdv`.`user_id` =`user`.`id`
        WHERE `type_pec`.`prat_id` = $id
        AND `rdv`.`heure_rdv` LIKE '$date%'";
        $arr[] = connexion::query_fetch_all($sql);
        return $arr;
